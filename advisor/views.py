...
def summarize_long_text(text, chunk_size=4000):
    """Summarize long text by chunking and summarizing each chunk via the model."""
    model = genai.GenerativeModel("gemini-2.5-flash")
    chunks = [text[i:i+chunk_size] for i in range(0, len(text), chunk_size)]
    summaries = []
    for c in chunks:
        prompt = f"Summarize the following CV text chunk in 2-3 clear sentences:\n\n{c}"
        try:
...
def home(request):
    ...
    upload_form = UploadCVForm()

    # check for uploaded file stored in session
    uploaded_file = None
    ufid = request.session.get('uploaded_file_id')
    if ufid:
        try:
            uploaded_file = UploadedFile.objects.get(id=ufid)
    ...
    if request.method == "POST":
        ...
        # Upload action: save file and store id in session, then redirect to show preview
        if action == 'upload':
            upload_form = UploadCVForm(request.POST, request.FILES)
            if upload_form.is_valid() and upload_form.cleaned_data.get('file'):
                ...
                # fall through and render form errors
                pass

        # Analyze action: analyze already-uploaded file (from session) and send to AI
        elif action == 'analyze':
            ufid = request.session.get('uploaded_file_id')
            if not ufid:
                # nothing to analyze
                Message.objects.create(conversation=conversation, sender='ai', text='No file uploaded to analyze.')
                return redirect('home')
            try:
                uploaded = UploadedFile.objects.get(id=ufid)
            except UploadedFile.DoesNotExist:
                Message.objects.create(conversation=conversation, sender='ai', text='Uploaded file not found.')
                request.session.pop('uploaded_file_id', None)
                return redirect('home')

            # perform analysis (extract -> chunk/summarize -> prompt)
            extracted = extract_text_from_file(uploaded.file.path)
            if not extracted:
                Message.objects.create(conversation=conversation, sender='ai', text='Failed to extract text from the uploaded file.')
                return redirect('home')

            # summarize/chunk if needed
            if len(extracted) > 8000:
                summary = summarize_long_text(extracted)
                prompt_text = f"Please analyze the user's CV below (summary first, then key points):\n\nSummary:\n{summary}\n\nFull CV (truncated):\n{extracted[:12000]}\n\nProvide suggestions for improvements, structure, and a short roadmap."
            else:
                prompt_text = f"Please analyze the user's CV below and suggest improvements, structure, and a short roadmap:\n\n{extracted}"

            # Save a user message noting analysis request
            Message.objects.create(conversation=conversation, sender='user', text=f'Analyze uploaded CV: {uploaded.file.name}')
            ...
            # keep uploaded_file in session for further analyzes or remove if you want
            return redirect('home')

        # Regular send action (text message)
        else:
    ...
    return render(request, "advisor/home.html", {"conversation": conversation, "messages": messages, "conversations": conversations, "upload_form": upload_form, 'uploaded_file': uploaded_file})


def extract_text_from_file(path):
    """Try to extract text from common CV formats: txt, pdf, docx.
    Requires `python-docx` for docx and `PyPDF2` for PDFs to be installed. Falls back to None on failure."""
    import os
